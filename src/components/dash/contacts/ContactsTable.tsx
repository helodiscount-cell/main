...
export const ContactsTable = () => {
  ...
  if (isError) {
    return (
      <div className="bg-white rounded-xl flex-1 mt-4 flex flex-col items-center justify-center p-12 border border-red-100">
        ...
        <button
          onClick={() => refetch()}
          className="mt-4 px-4 py-2 bg-slate-100 text-slate-700 rounded-lg text-[16px] hover:bg-slate-200 transition-colors"
        >
          Try Again
        </button>
      </div>
  ...
  return (
    <div className="bg-white rounded-xl overflow-hidden flex-1 flex flex-col border border-slate-100">
      {/* Table header */}
      <div
        className="grid grid-cols-[1.5fr_0.8fr_auto_1.5fr_auto_1fr] items-center border-b border-slate-100 m-4 p-4 rounded-lg"
        style={{ background: "whitesmoke" }}
      >
        <span className="text-[16px] font-medium text-[#212121]">
          {CONTACTS_CONFIG.COLUMNS[0].label}
        </span>
        <span className="text-[16px] font-medium text-[#212121] text-center">
        ...
        <div className="flex items-center gap-2 justify-center">
          ...
          <div className="bg-slate-900 rounded-sm p-0.5 mt-0.5">
      ...
      <div className="flex-1 overflow-auto">
        {contacts.length === 0 ? (
          <div className="flex items-center justify-center py-16 text-[16px] text-slate-400">
            {CONTACTS_CONFIG.EMPTY_STATE_MESSAGE}
          </div>
        ) : (
          contacts.map((contact, index) => (
            <div key={contact.id}>
              <div className="grid grid-cols-[1.5fr_0.8fr_auto_1.5fr_auto_1fr] bg-[#F9F9F9] items-center px-6 py-4 group hover:bg-slate-50/50 transition-colors">
                {/* Username Column */}
                ...
                <span className="text-[16px] text-[#212121] text-center font-medium">
